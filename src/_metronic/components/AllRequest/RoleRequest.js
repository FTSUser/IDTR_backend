...
const RoleRequest = ({ getNewCount, title }) => {
    ...
    const getAllCourseName = async () => {
        ...
        if (!search) {
            await ApiGet(
                `request/getRequestRole?page=${page}&limit=${countPerPage}`
            )
                .then((res) => {
            ...
            await ApiGet(
                `request/getRequestRole?search=${search}&page=${page}&limit=${countPerPage}`
            )
                .then((res) => {
                    setIsLoaderVisible(false);
                    setFilteredCourseName(res?.data?.payload?.Request);
