...
const Facilities = ({ getNewCount, title }) => {
  ...
  const [idForUpdateHelpfulTipsData, setIdForUpdateHelpfulTipsData] =
    useState("");
  ...
  const getAllHelpfulTips = async () => {
    ...
    if (!search) {
      await ApiGet(`facility/getAllFacility?page=${page}&limit=${countPerPage}`)
        .then((res) => {
          setIsLoaderVisible(false);
          setFilteredHelpfulTips(res?.data?.payload?.Facility);
          setCount(res?.data?.payload?.count);
        })
        .catch((err) => {});
    } else {
      await ApiGet(
        `facility/getAllFacility?search=${search}&page=${page}&limit=${countPerPage}`
      )
        .then((res) => {
          setIsLoaderVisible(false);
          setFilteredHelpfulTips(res?.data?.payload?.Facility);
  ...
  const validateFormForAddAdmin = () => {
    ...
    if (inputValueForAdd && !inputValueForAdd.content) {
      formIsValid = false;
      errorsForAdd["content"] = "*Please Enter content!";
    }
  ...
  const handleAddHelpfulTipsDetails = (e) => {
    ...
    if (validateFormForAddAdmin()) {
      let Data = {
        title: inputValueForAdd.titleName,
  ...
  const validateForm = () => {
    ...
    if (inputValue && !inputValue.content) {
      formIsValid = false;
  ...
  const handleDeleteHelpfulTips = () => {
    ApiDelete(`facility/deleteFacility/${idForDeleteHelpfulTips}`)
      .then((res) => {
        if (res?.status == 200) {
          setShow(false);
  ...
  const removeImage = (data) => {
    setInputValueImage(inputValueImage.filter((item) => item !== data));
  };
  ...
  const handleUpdateHelpfulTipsDetails = (e) => {
    ...
    if (validateForm()) {
      let Data = {
        title: inputValue.titleName,
        description: description,
        image: inputValueImage,
        content: inputValue.content,
        // language: inputValue.language,
      };
      ApiPut(`facility/updateFacility/${idForUpdateHelpfulTipsData}`, Data)
        .then((res) => {
          if (res?.status == 200) {
            setIsUpdateHelpfulTips(false);
            toast.success(res?.data?.message);
            ...
            <div className="d-flex justify-content-between">
              <div
                ...
                onClick={() => {
                  ...
                  setInputValueImage(row?.image);

                  setDescription(row?.description);
                }}
              >
                <Tooltip title="Edit Facility" arrow>
                  <CreateIcon />
                </Tooltip>
              </div>
            </div>
        ...
        <div className="p-2 mb-2">
          <div className="row mb-4 pr-3">
            <div className="col d-flex justify-content-between">
              <h2 className="pl-3 pt-2"> Facility</h2>
            </div>
            <div className="col">
              <div>
                <input
          ...
          <Modal show={show} onHide={handleClose}>
            ...
            <Modal.Body>Are You Sure To Want To delete this Facilty</Modal.Body>
            <Modal.Footer>
                  ...
                  <div className="col-lg-9 col-xl-6">
                    <div>
                      <input
                        ...
                        id="content"
                        name="content"
                        value={inputValueForAdd.content}
                        onChange={(e) => {
                          handleOnChnageAdd(e);
                        }}
                      />
                ...
                <div className="honda-text-grid12 honda-text-grid-border">
                  ...
                  <div className="honda-text-grid-items">
                    ...
                    <div className="" style={{ display: "flex" }}>
                      {dataViewMore?.image.map((data, index) => (
                        <div key={index}>
