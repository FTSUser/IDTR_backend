...
const Banner = ({ getNewCount, title }) => {
  ...
  const getAllAnnouncement = async () => {
    ...
    if (!search) {
      await ApiGet(
        `banner/getAllBanner?page=${page}&limit=${countPerPage}`
      )
        .then((res) => {
...
console.log(file, "fileData")
  ...
  const handleDeleteAnnouncement = () => {
    ApiDelete(`banner/deleteBanner/${idForDeleteAnnouncement}`)
      .then((res) => {
        if (res?.status == 200) {
          setShow(false);
  ...
  const handleUpdateAnnouncementDetails = (e) => {
    ...
    if (validateForm()) {
      ...
      ApiPut(
        `banner/updateBanner/${idForUpdateAnnouncementData}`,
        Data
      )
        .then((res) => {
          if (res?.status == 200) {
